...
public class ReportService {
    ...
    public void generate(final String filename, final String location) throws ReportGenerationProblemException, InvalidLocationException {
        ...
        try (Document document = new Document()) {
            ...
            document.add(new Paragraph(resourceController.getByKey("location", ResourceController.Case.CAPITALIZED_CASE) + ": " + weather.getLocation()));
            document.add(new Paragraph(resourceController.getByKey("temperature", ResourceController.Case.CAPITALIZED_CASE) + ": " + weather.getTemperature() + "°C"));
            document.add(new Paragraph(resourceController.getByKey("humidity", ResourceController.Case.CAPITALIZED_CASE) + ": " + weather.getHumidity() + "%"));
            document.add(new Paragraph(resourceController.getByKey("windSpeed", ResourceController.Case.CAPITALIZED_CASE) + ": " + weather.getWindSpeed() + " m/s"));
            document.add(new Paragraph(resourceController.getByKey("pressure", ResourceController.Case.CAPITALIZED_CASE) + ": " + weather.getPressure() + " hPa"));
        } catch (IOException e) {
